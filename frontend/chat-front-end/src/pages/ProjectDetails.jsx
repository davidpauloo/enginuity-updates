...
const ProjectDetailsPage = () => {
    ...
    const handleAddActivity = async (e) => {
        ...
        try {
            ...
            console.log('Sending activity data:', JSON.stringify(activityData, null, 2)); // Debug: Confirm payload

            const response = await axiosInstance.post(`/projects/${projectId}/activities`, activityData);
            console.log('Server response for add activity:', response.data); // Debug: Check server response
            ...
            if (response.data && response.data.activities) { // Assuming response.data is the updated project
                setProject(response.data); // Directly set project from response.data
                setActivities(response.data.activities); // Update activities from the project object
                toast.success("Activity added successfully");
            } else {
                // Fallback if backend doesn't return full project (shouldn't happen with correct backend)
                fetchProjectDetails(); // Re-fetch to ensure consistency
    ...
    const getUpcomingDeadlines = () => {
        ...
        return activities
            .filter(activity => {
                const dueDate = new Date(activity.dueDate); // Use activity.dueDate
                dueDate.setHours(0, 0, 0, 0);
                return dueDate >= today && activity.status !== 'completed';
            })
            .sort((a, b) => new Date(a.dueDate) - new Date(b.dueDate)) // Use activity.dueDate
            .slice(0, 5);
    };
    ...
    const handleAddEmployee = async (e) => {
        ...
        try {
            ...
            const employeeData = {
                ...
                dueDate: employeeEndDateIso // Backend expects 'dueDate' for employee's end date
            };

            console.log('Sending employee data:', JSON.stringify(employeeData, null, 2)); // Debug: Confirm payload

            const response = await axiosInstance.post(`/projects/${projectId}/employees`, employeeData);
            console.log('Server response for add employee:', response.data); // Debug: Check server response

            // Backend should return the entire updated project object
            if (response.data && response.data.employees) { // Assuming response.data is the updated project
                setProject(response.data); // Update project state with the new employee
    ...
    const handleRemoveEmployee = async () => {
        ...
        try {
            ...
            if (response.data && response.data.employees) { // Assuming response.data is the updated project
                ...
                fetchProjectDetails(); // Fallback re-fetch
        ...
        <div className="p-0 sm:p-0 md:p-0 lg:p-0 xl:p-0 2xl:p-0">
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                                            ...
                                            <li key={activity.id || activity._id} className="flex items-start gap-3 p-3 bg-base-200 rounded-lg">
                                                ...
                                                <div className="flex-1">
                                                    <div className="flex items-center justify-between">
                                                        {/* Use activity.name if that's what your backend provides for the activity name */}
                ...
                <div className="lg:w-1/3 space-y-8">
                    ...
                    <div className="card bg-base-100 shadow-md">
                        <div className="card-body">
                            <h3 className="text-xl font-semibold text-base-content mb-4">Financial Overview</h3>
                            <div className="flex items-center justify-between">
                                <span className="text-base-content/80">Budget:</span>
                                <span className="font-bold text-lg text-info">₱{project.budget ? project.budget.toLocaleString() : 'N/A'}</span>
                            </div>
                            <div className="flex items-center justify-between mt-2">
                                <span className="text-base-content/80">Expenses:</span>
                                <span className="font-bold text-lg text-warning">₱{project.expenses ? project.expenses.toLocaleString() : 'N/A'}</span>
                            </div>
                            <div className="flex items-center justify-between mt-2">
                                <span className="text-base-content/80">Remaining:</span>
                                <span className="font-bold text-lg text-success">₱{project.budget && project.expenses ? (project.budget - project.expenses).toLocaleString() : 'N/A'}</span>
                            </div>
                        </div>
                    </div>
